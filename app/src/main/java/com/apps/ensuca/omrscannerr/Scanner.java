...
public class Scanner {
    ...
    public void scan() throws Exception {
        ...
        if(logging) Util.write2File(adaptiveThresh, "step_6_adaptive_thresh.png");
    ...
    private void findParentRectangle() throws Exception {
        ...
        if(logging) Util.sout("getParentRectangle > hiearchy data:\n" + hierarchy.dump());
        ...
        if(logging) Util.sout("getParentRectangle > contours.size: " + contours.size());
        if(logging) Util.sout("getParentRectangle > rectangles.size: " + rectangles.size());
...
//            double childId = ids[2];
            ...
            if(logging) Util.sout("getParentRectangle > index: " + index + ", c: " + c);
        }

        if(logging) Util.sout("getParentRectangle > parentIndex: " + parentIndex);
        ...
        if(logging) Util.sout("getParentRectangle > original roi.x: " + roi.x + ", roi.y: " + roi.y);
        if(logging) Util.sout("getParentRectangle > original roi.width: " + roi.width + ", roi.height: " + roi.height);
        ...
        if(logging) Util.sout("getParentRectangle > modified roi.x: " + roi.x + ", roi.y: " + roi.y);
        if(logging) Util.sout("getParentRectangle > modified roi.width: " + roi.width + ", roi.height: " + roi.height);

        if(logging) Util.write2File(source.submat(roi), "step_7_roi.png");
    }
    ...
    private void findBubbles() throws Exception {
        ...
        if(logging) Util.sout("findBubbles > ideal circle size > minThreshold: " + minThreshold + ", maxThreshold: " + maxThreshold);
        ...
        for(MatOfPoint contour : contours){
            ...
            if(logging) Util.sout("findBubbles > founded circle > w: " + w + ", h: " + h);
        ...
        if(logging) Util.sout("findBubbles > bubbles.size: " + drafts.size());
...
//                if(logging) write2File(conjuction, "conjuction_" + i + "_" + j + ".png");
                ...
                if(logging) Util.sout("recognizeAnswers > " + i + ":" + j + " > countNonZero: " + countNonZero);
            ...
            if(logging) Util.sout("recognizeAnswers > selection is " + (selection == null ? "empty/invalid" : selection[2]));
